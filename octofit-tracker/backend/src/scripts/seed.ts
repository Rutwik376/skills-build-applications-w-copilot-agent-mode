...
async function seedDatabase() {
  try {
    ...
    await User.findByIdAndUpdate(alexUser._id, { teamId: teams[0]._id });
    await User.findByIdAndUpdate(taylorUser._id, { teamId: teams[0]._id });
    await User.findByIdAndUpdate(jordanUser._id, { teamId: teams[1]._id });
    await User.findByIdAndUpdate(morganUser._id, { teamId: teams[2]._id });
    await User.findByIdAndUpdate(samUser._id, { teamId: teams[2]._id });
    await User.findByIdAndUpdate(caseyUser._id, { teamId: teams[2]._id });
    ...
    for (const activity of activities) {
      console.log(
        `  - ${activity.type} (${activity.duration}min, +${activity.activityPoints}pts)`
      );
    }
    ...
    // Display user points
    console.log(`\n🏆 User Points:`);
    const updatedUsers = await User.find().sort({ totalActivityPoints: -1 });
    for (let i = 0; i < updatedUsers.length; i++) {
      console.log(
        `  ${i + 1}. ${updatedUsers[i].username}: ${updatedUsers[i].totalActivityPoints} points`
      );
    }
